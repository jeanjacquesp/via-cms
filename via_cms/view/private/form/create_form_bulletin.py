...
class CreateFormBulletin(FlaskForm):
    ...
    position = TextAreaField(_l('Position'),
                             render_kw={'rows': '2', 'data-label': _l('Position'), 'placeholder': _l('Position')},
                             validators=[Length(max=255)])
    ...
    vaccination_age = TextAreaField(_l('Age'),
                                    render_kw={'rows': '1', 'data-label': _l('Age'),
                                               'placeholder': _l('Age')},
                                    validators=[Length(max=70)])
    # Events form
    ...
    body_json = TextAreaField(_l('Body in json format'), render_kw={
    ...
    contact_json = TextAreaField(_l('Contact information as a json string'), render_kw={
        'rows': '4', 'data-label': _l('Contact')}, validators=[Optional(), Length(max=1000)])
    ...
    def __init__(self, subject_name):
        super().__init__()
        profile_news_id = Profile.query.filter_by(name='bulletin').first().id
        lang = get_locale()
